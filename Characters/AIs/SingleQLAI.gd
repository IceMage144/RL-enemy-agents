...
func init(params):
	.init(params)
	self.ep = Experience.new(self.experience_pool_size, self.weight_exponent)
...
func _compute_value_from_q_values(state):
	...
	for a in legal_actions:
		q_values_list.append(self._get_q_value(state, a, self.learning_model))
	if not self._is_freezing_weights():
		return global.max(q_values_list)
	var max_action = legal_actions[global.argmax(q_values_list)]
	return self._get_q_value(state, max_action, self.freezed_model)
...
func _update_weights_experience(feat_sample, reward_sample, next_sample, pos_list):
	...
	for i in range(feat_sample.size()):
		var next_q_val = self._compute_value_from_q_values(next_sample[i])
		var label = reward_sample[i] + self.discount * next_q_val
	...
	var weights = self.ep.update(pos_list, priorities)
