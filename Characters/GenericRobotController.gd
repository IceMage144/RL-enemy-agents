...
func get_legal_actions(state):
	var fl = global.find_entity("floor")
	...
	for dir in Action.directions(true):
		if fl == null:
			...
			var act_dir = Action.to_vec(dir)
			var cell_pos = fl.world_to_map(state.self_pos) + act_dir
			var cell = fl.get_cellv(cell_pos)
			if cell != 1:
				legal_actions.append(Action.compose(Action.WALK, dir))
...
# 	var out = self.get_features(state)
...
# 			out[Feature.ENEMY_DIST] = new_dist / diag
...
# 	if self_mov == Action.ATTACK:
# 		if self._is_aligned(state.self_act, pos_diff):
# 			out[Feature.ENEMY_LIFE] -= state.self_damage * damage_chance / state.enemy_maxlife
...
# 		out[Feature.SELF_LIFE] -= state.enemy_damage * damage_chance / state.self_maxlife

# 	out[Feature.POS_X_DIFF] -= self_dir_vec.x / diag
# 	out[Feature.POS_Y_DIFF] -= self_dir_vec.y / diag
